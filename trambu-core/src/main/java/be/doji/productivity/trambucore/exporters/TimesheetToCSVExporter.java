...
public class TimesheetToCSVExporter implements Exporter<List<ActivityLog>, List<String>> {
  ...
  @Override
  public List<String> convert(List<ActivityLog> input) throws IOException {
  ...
  private String createItemLine(ActivityLog timeLog) {
    Optional<Activity> savedActivityById = activityManager
        .getSavedActivityById(timeLog.getActivityId().toString());
    ...
    Optional<Activity> parentActivity = savedActivityById
        .flatMap(activity -> activityManager.getSavedActivityById(activity.getParentActivity()));
    String parentActivityString = parentActivity.isPresent()
        ? parentActivity.get().getName()
        : ExportConstants.CSV_ITEM_EMPTY_VALUE;

    csvLine.append(
        savedActivityById.map(activity -> parentActivityString)
            .orElse(ExportConstants.CSV_ITEM_EMPTY_VALUE));
