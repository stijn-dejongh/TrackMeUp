...
public class ActivityView extends TitledPane {
  ...
  @NotNull
  private StackPane createContentContainer() {
  ...
  GridPane createLogPoints() {
    ...
    logpointGrid
        .add(new Label(presenter.getActivityLog().getTimeSpentInHoursString()), 1, logRowIndex);
    return logpointGrid;
  }
  ...
  private Button createNotes() {
    ...
    noteButton
        .setTooltip(DisplayUtils.createTooltip(TooltipConstants.TOOLTIP_TEXT_ACTIVITY_NOTE_EXPAND));
  ...
  private Node createParentSelector() {
    ObservableList<String> options = FXCollections
        .observableArrayList(presenter.getPossibleParents());
  ...
  private void initDeleteButton() {
    ...
    deleteButton
        .setTooltip(DisplayUtils.createTooltip(TooltipConstants.TOOLTIP_TEXT_ACTIVITY_DELETE));
  }
