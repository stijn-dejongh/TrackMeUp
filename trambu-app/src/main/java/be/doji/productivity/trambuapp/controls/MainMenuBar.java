...
public class MainMenuBar extends UIComponent {
    ...
    activityPage.setOnAction(event -> {
      if (!superView.getClass().equals(ActivityPageView.class)) {
        superView.replaceWith(JvmClassMappingKt.getKotlinClass(ActivityPageView.class),
            new ViewTransition.Slide(Duration.seconds(0.3), ViewTransition.Direction.LEFT), true,
            true);
      }
    });
    activityPage
        .setTooltip(DisplayUtils.createTooltip(TooltipConstants.TOOLTIP_TEXT_MENU_ACTIVITIES));
    return activityPage;
  }
  ...
  private Button createTimesheetButton() {
    ...
    timesheetPage
        .setTooltip(DisplayUtils.createTooltip(TooltipConstants.TOOLTIP_TEXT_MENU_TIMESHEET));
