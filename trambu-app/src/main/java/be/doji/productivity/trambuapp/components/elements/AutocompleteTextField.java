...
public class AutocompleteTextField extends TextField {
    ...
    if (StringUtils.isBlank(currentText)) {
      ...
      List<String> matchingSuggestions = suggestions.stream()
          .filter(suggestion -> suggestion.toLowerCase()
              .contains(getLatestEntry(currentText).toLowerCase()))
  ...
  @NotNull
  private String getLatestEntry(String currentText) {
    String afterIndicator = StringUtils
        .substringAfter(currentText, DisplayConstants.SEPARATOR_TAGS_PROJECTS)
        .trim();
