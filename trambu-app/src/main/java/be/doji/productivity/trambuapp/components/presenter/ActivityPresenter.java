...
public class ActivityPresenter extends Presenter {
  ...
  private void refreshViewStyle() {
    view.getStyleClass()
        .removeAll(DisplayConstants.STYLE_CLASS_ACTIVITY_DONE,
  ...
  private FontAwesomeIconView getHeaderIcon() {
    FontAwesomeIconView checkedCalendar = DisplayUtils
        .createStyledIcon(FontAwesomeIcon.CHECK_CIRCLE);
    FontAwesomeIconView uncheckedCalendar = DisplayUtils
        .createStyledIcon(FontAwesomeIcon.CIRCLE_ALT);
  ...
  public void openNotes() {
    try {
      ...
      textField
          .setText(note.getContent().stream().collect(Collectors.joining(System.lineSeparator())));
  ...
  public void changeParent(String newParent) {
    Optional<Activity> savedParent = this.managerContainer.getActivityManager()
        .getSavedActivityByName(newParent);
    savedParent.ifPresent(
        activity -> this.managerContainer.getActivityManager()
            .addActivityAsSub(this.model, activity));
  ...
  @NotNull
  private SortedSet<String> getProjectSuggestions() {
  ...
  @NotNull
  private SortedSet<String> getTagSuggestions() {
    SortedSet<String> treeSetProjects = new TreeSet<>();
  ...
  @NotNull
  private SortedSet<String> getLocationSuggestions() {
    SortedSet<String> treeSetProjects = new TreeSet<>();
  ...
  public String getEditButonText() {
    return this.isEditable() ? DisplayConstants.BUTTON_TEXT_SAVE
        : DisplayConstants.BUTTON_TEXT_EDIT;
  }
  ...
  public FontAwesomeIconView getTimingButtonIcon() {
    return DisplayUtils.createStyledIcon(
        activityLog.getActiveLog().isPresent() ? FontAwesomeIcon.HOURGLASS_END
